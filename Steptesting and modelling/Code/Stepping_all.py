...
def plotsteps():
    ...
    for i,input in enumerate(stepped_vars):
        for j, output in enumerate(outputs):
            plot.figure()
            plot.plot(tspan,step_sols[i][output] - np.ones_like(step_sols[i][output]) * ss_vals[output])
...
def get_results():
    ...
    for i, input in enumerate(stepped_vars):
        for j, output in enumerate(outputs):
            ...
            responses.append(step_sols[i][output] - np.ones_like(step_sols[i][output]) * ss_vals[output])
