...
def plotsteps():
    from steady_state_values import steady_state
    ss_vals = steady_state()
    ss_vals['Cc_measured'] = ss_vals['Cc']
    import numpy as np
    stepped_vars = ['Ps1','Ps2','Ps3','Cao','Tbo','F1']
    step_sols = [run_sim(i) for i in stepped_vars]
    inputsteps = [20,20,20,0.2*7.4,0.2*24 ,0.2* 7.334e-4]
    outputs = ['Cc_measured','T','H']
    inputstep = dict(zip(stepped_vars,inputsteps))
    outputs = ['Cc_measured','T','H']

    for i,input in enumerate(stepped_vars):
        for j, output in enumerate(outputs):
            plot.figure()
            plot.plot(tspan,(step_sols[i][output] - np.ones_like(step_sols[i][output]) * ss_vals[output])/(inputstep[input]*np.ones_like(tspan)))
...
#/(inputstep[input]*np.ones_like(tspan)))
